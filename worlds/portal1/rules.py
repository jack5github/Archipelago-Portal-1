...
def set_all_rules(world: Portal1World) -> None:
    set_all_entrance_rules(world)
    set_all_location_rules(world)
    set_completion_condition(world)


def set_all_entrance_rules(world: Portal1World) -> None:
    """
    ...
    set_rule(
        right_room_to_final_boss_room,
        lambda state: state.has("Top Left Room Button Pressed", world.player),
    )
    ...
    if world.options.hammer:
        ...
        set_rule(
            overworld_to_top_middle_room,
            lambda state: state.has("Hammer", world.player),
        )
    """


def set_all_location_rules(world: Portal1World) -> None:
    """
    ...
    set_rule(
        ...
        lambda state: (
            ...
            and (
                not world.options.hard_mode
                or state.has_any(("Shield", "Health Upgrade"), world.player)
            )
        ),
    )
    # DON'T DO THIS!!!!
    ...
    if world.options.hard_mode:
        ...
        set_rule(
            ...
            lambda state: (
                state.has("Sword", world.player)
                and state.has_any(("Shield", "Health Upgrade"), world.player)
            ),
        )
    else:
